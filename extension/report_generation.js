...
async function generateReport({
  ...
  settings = {},
  onProgress = null,
  endpointPath = "/v1/report",
  readyPhase = "report_ready",
  callingPhase = "calling_report_backend"
}) {
  ...
  const response = await fetchImpl(`${reportSettings.backendBaseUrl}${endpointPath}`, {
    method: "POST",
    headers: {
...
async function generateGist({
  ...
  return generateReport({
    ...
    settings,
    onProgress,
    endpointPath: "/v1/gist",
    readyPhase: "gist_ready",
    callingPhase: "calling_gist_backend"
  });
}
