...
test("background controller generates a gist through the configured endpoint", async () => {
  const chromeStub = createChromeStub();
  const calls = [];
  const controller = background.createBackgroundController({
    chromeApi: chromeStub,
  ...
  try {
    const phases = [];
    const report = await controller.generateGist({
      path: [{ id: "1", text: "Root" }],
      references: [],
      people: [],
      replyChains: []
    ...
    assert.equal(calls[1].url, "http://127.0.0.1:8787/v1/gist");
  } finally {
    require("../extension/dev_env_loader.js").loadGeneratedConfig = originalLoadGeneratedConfig;
  }
});
