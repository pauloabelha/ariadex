...
test("buildExportFilename uses the clicked tweet id when available", () => {
  assert.equal(content.buildExportFilename("123"), "ariadex-123.json");
  assert.equal(content.buildExportFilename(""), "ariadex-root-path.json");
});

test("buildReportFilename uses the clicked tweet id when available", () => {
  assert.equal(content.buildReportFilename("123"), "ariadex-123-report.md");
  assert.equal(content.buildReportFilename(""), "ariadex-root-path-report.md");
});
...
test("generateGistArtifact sends the expected extension message", async () => {
  const chromeStub = {
    runtime: {
      lastError: null,
      sendMessage(message, callback) {
  ...
  const report = await content.generateGistArtifact({
    path: [{ id: "1" }],
    references: [],
    people: [],
    replyChains: []
  ...
  assert.deepEqual(chromeStub.sent, {
    type: content.GENERATE_GIST_MESSAGE_TYPE,
    artifact: {
      path: [{ id: "1" }],
      references: [],
      people: [],
...
test("generateGistArtifact uses the streaming port when available and relays progress", async () => {
  const progressEvents = [];
  let portListener = null;
  let disconnectListener = null;
  const chromeStub = {
    runtime: {
      lastError: null,
      connect({ name }) {
        assert.equal(name, content.GENERATE_GIST_PORT_NAME);
        return {
          onMessage: {
            addListener(listener) {
              portListener = listener;
  ...
  assert.equal(report.text, "Portable gist text.");
});
...
test("renderGistTab shows generated gist text and metadata", async () => {
  const clipboardWrites = [];
  const root = {
    defaultView: {
      navigator: {
  ...
  const tab = content.renderGistTab({
    text: "Portable gist.",
    model: "gemma-test",
    generatedAt: "2026-04-30T00:00:00.000Z"
  }, root);
  ...
  const article = tab.children[0];
  const topRow = article.children[0];
  assert.equal(topRow.children[0].textContent, "Portable Gist");
  const actions = topRow.children[1];
  assert.equal(actions.children[0].title, "Copy Gist");
  assert.equal(actions.children[1].textContent, "Download");
  assert.equal(actions.children[1].title, "Download Gist");
  assert.match(article.children[1].textContent, /Generated 2026-04-30T00:00:00.000Z/);
  assert.match(article.children[1].textContent, /model gemma-test/);
  assert.equal(article.children[2].textContent, "Portable gist.");
  await actions.children[0].onclick?.();
...
test("resolveRootArtifact uses the streaming port when available and relays progress", async () => {
  ...
  const chromeStub = {
    runtime: {
      sendMessage() {},
      connect(options) {
        assert.deepEqual(options, { name: "ARIADEX_RESOLVE_ROOT_PATH_PORT" });
        return {
          onMessage: {
            addListener(listener) {
              portListener = listener;
