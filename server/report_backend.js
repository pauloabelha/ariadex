...
function loadPrompt(promptPath = REPORT_PROMPT_PATH) {
  return fs.readFileSync(promptPath, "utf8");
}

function resolvePromptPathname(requestUrl = "") {
  const pathname = String(requestUrl || "").trim();
  if (pathname === "/v1/gist") {
    return GIST_PROMPT_PATH;
  }
  return REPORT_PROMPT_PATH;
}
...
function createServer({ fetchImpl, env = null, repoConfig = null } = {}) {
  ...
  return http.createServer(async (req, res) => {
    ...
    const promptPath = resolvePromptPathname(req.url || "");
    if (req.method !== "POST" || !promptPath) {
